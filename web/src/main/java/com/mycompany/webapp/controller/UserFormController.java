...
public class UserFormController extends BaseFormController {
	...
	public String onSubmit(@ModelAttribute("user") final User user,
		...
		if (request.getParameter("delete") != null) {
			...
			if (!StringUtils.equals(request.getParameter("from"), "list")) {
				...
				if (StringUtils.isBlank(request.getParameter("version"))) {
					...
					try {
						final String resetPasswordUrl = getUserManager()
								.buildRecoveryPasswordUrl(
										user,
	...
	protected ModelAndView showProfile(final HttpServletRequest request,
			final HttpServletResponse response) throws Exception {
		Model model = new ExtendedModelMap();
		return new ModelAndView("profile", model.asMap());
	}

	@RequestMapping(value = "/mu/favoriteUser*", method = RequestMethod.POST)
    public @ResponseBody String favoriteUser(@RequestParam("userId") final String userId) throws Exception {
		System.out.println("user id "+userId);
        Model model = new ExtendedModelMap();
        User profile = getUserManager().favoriteUser(userId);
